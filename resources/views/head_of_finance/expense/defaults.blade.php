            ...
            <table class="w-full text-xs">
                <thead class="bg-slate-100 text-gray-600 uppercase">
                    <tr>
                        ...
                        <th class="px-3 py-2 text-right w-20">ຈ/ນ ເດືອນ</th>
                ...
                <tbody id="sortable_{{ $catId }}"
                    ...
                    <tr class="hover:bg-gray-50 select-none" data-id="{{ $d->id }}">
                        ...
                        <td class="px-3 py-2 text-gray-800">{{ $d->item_name }}</td>
                        <td class="px-3 py-2 text-center text-gray-400">{{ $d->reference }}</td>
                        ...
                        <td class="px-3 py-2 text-right font-medium text-gray-700">{{ number_format($d->amount_per_month * $d->num_months, 0, '.', ',') }}</td>
                        <td class="px-3 py-2 text-center">
                            <div class="flex items-center justify-center gap-2">
                                ...
                                <button type="button"
                                    onclick="openDeleteModal('{{ route('head_of_finance.expense.defaults.destroy', $d) }}', '{{ addslashes($d->item_name) }}')"
...
function renumberRows(tbody) {
    let i = 1;
    tbody.querySelectorAll('tr:not(.empty-row) .row-num').forEach(cell => { cell.textContent = i++; });
}
