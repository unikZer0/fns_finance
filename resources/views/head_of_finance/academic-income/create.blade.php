...
@section('title', 'ສ້າງແຜນລາຍຮັບວິຊາການ')
@section('page-title', 'ສ້າງແຜນລາຍຮັບວິຊາການໃໝ່')

@section('content')
    <div class="max-w-lg mx-auto bg-white rounded-lg shadow-sm p-8">
        <h2 class="text-lg font-semibold text-gray-800 mb-6">ຂໍ້ມູນແຜນລາຍຮັບວິຊາການ</h2>

        <form action="{{ route('head_of_finance.academic_income.store') }}" method="POST" class="space-y-5">
            @csrf

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">
                    ສົກງົບປະມານ (ຕົວຢ່າງ: 2026) <span class="text-red-500">*</span>
                </label>
                <input type="number" name="fiscal_year"
                    value="{{ old('fiscal_year', now()->year + 1) }}"
                    min="2000" max="9999"
