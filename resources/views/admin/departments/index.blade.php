...
<div class="table-wrapper">
    ...
    <div class="filter-bar">
        <form id="filter-form" method="GET" action="{{ route('admin.departments.index') }}" style="display:flex; flex-wrap:wrap; gap:12px; width:100%;">
            ...
            <select name="department_type" onchange="this.form.submit()" class="form-input" style="width:auto; min-width:160px;">
        ...
        <tbody>
            @forelse ($departments as $dept)
                <tr>
                    ...
                    <td><span class="badge badge-gray">{{ $dept->users_count ?? $dept->users()->count() }} ຄົນ</span></td>
                    <td style="text-align:right">
                        <div style="display:flex; align-items:center; justify-content:flex-end; gap:4px;">
                            <a href="{{ route('admin.departments.show', $dept) }}" class="btn-icon" title="ເບິ່ງລາຍລະອຽດ">
                                <svg style="width:15px;height:15px" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                            </a>
                            <a href="{{ route('admin.departments.edit', $dept) }}" class="btn-icon" title="ແກ້ໄຂ">
                            ...
                            <button type="button" onclick="openDeleteModal('{{ route('admin.departments.destroy', $dept) }}', '{{ $dept->department_name }}')" class="btn-icon" title="ລົບ" style="color:#DC2626;">
    ...
    @if ($departments->hasPages())
        <div style="padding:16px 20px; border-top:1px solid var(--color-border);">{{ $departments->links() }}</div>
    @endif
</div>
...
<div id="deleteModal" class="modal-overlay" style="display:none;">
    <div class="modal" style="max-width:400px;">
        <div class="modal-body" style="text-align:center; padding:28px 24px;">
            ...
            <p style="font-size:var(--font-size-base);color:var(--color-text-secondary);margin-bottom:8px;">ທ່ານແນ່ໃຈບໍ່ວ່າຈະລົບພະແນກນີ້?</p>
