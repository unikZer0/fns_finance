...
- `app/Http/Controllers/HeadOfFinance/BudgetInstallmentController.php` — **NEW** — จัดการแผนงวด (จัดสรรงวด 1 และ งวด 2 สำหรับแผนที่ APPROVED แล้ว พร้อม auto-calculate 6 เดือนและ 6 เดือนท้ายปี)
- Views: `resources/views/head_of_finance/annual-budget/{index,create,edit,show,preview,pdf}`
- Views: `resources/views/head_of_finance/budget-installment/{index,show}`
...
- `resources/views/components/admin-sidebar.blade.php` — Sidebar ตาม role: เมนู **ກວດສອບແຜນງົບປະມານ** แสดงเมื่อ user มี `reviewerAssignments` และไม่ใช่ deputy/head_of_faculty — เมนู **ພິຈາລະນາແຜນງົບປະມານ** สำหรับ deputy_head_of_faculty เพิ่ม **ອະນຸມັດແຜນງົບປະມານ** สำหรับ head_of_faculty
...
11. **แผนงวดงบประมาณ (Budget Period Allocations)** — **NEW** — จัดสรรงบประมาณรายงวด (งวด 1, งวด 2) สำหรับแผนที่เป็น APPROVED พร้อมการคำนวณ 6 เดือนต้นปี/ท้ายปี อัตโนมัติ
