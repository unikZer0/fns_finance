...
14. **Sidebar: ພາບລວມງົບປະມານ** — **NEW** — เมนูใน sidebar สำหรับ Head of Faculty ลิงก์ไปหน้า home (dashboard analytics)
15. **Plans placeholder (HoF)** — **NEW** — Route `/head-of-finance/plans` + `PlansCtrl` controller + placeholder view (ยังไม่มีเนื้อหา)
...
- **Dashboard analytics** — Home page ของทุก role ยังเป็นหน้าเปล่า/basic ไม่มีกราฟหรือสรุปข้อมูล **ยกเว้น Head of Faculty** ที่มี rich dashboard แล้ว (4 metric cards + progress bar + activity feed)
- **Layout inconsistency ใน home pages** — Head of Faculty home ใช้ `@extends('layouts.admin')` ในขณะที่ home pages อื่นใช้ `<x-app-layout>` — ควร standardize
...
- **Layout inconsistency** — `head_of_faculty/home.blade.php` ใช้ `@extends('layouts.admin')` + `@section('content')` ในขณะที่ home pages อื่นใช้ `<x-app-layout>` component — ควร standardize ให้เหมือนกัน
